...
> **Ověřování v tomhle projektu:** Web nemá automatický test suite — „doveď k zelené" tu znamená: **`npm run build` projde** + **vizuální kontrola na betě** (`beta.sfr-motor.cz`), případně F12 mobil. U úprav velkých souborů ověř i konec souboru a párování tagů (mount občas ustřihne konec při zápisu → edituj přes python read→replace→write).
